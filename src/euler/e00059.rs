...
fn decode_cipher(password: [u8; 3]) -> Vec<u8> {
    let l = password.len();

    CIPHER
        .iter()
        .enumerate()
        .map(|(i, b)| b ^ password[i % l])
        .collect()
}
...
pub fn solve() -> usize {
    ...
    // find most common char for each position and expect it to be the whitespace
    counts.clear();
    for c in CIPHER.iter().step_by(3) {
    ...
    counts.clear();
    for c in CIPHER.iter().skip(1).step_by(3) {
        *counts.entry(*c).or_default() += 1;
    }
    let max = counts.iter().max_by(|a, b| a.1.cmp(b.1)).unwrap();
    let key_1 = max.0 ^ b' ';
    ...
    counts.clear();
    for c in CIPHER.iter().skip(2).step_by(3) {
        *counts.entry(*c).or_default() += 1;
    }
    let max = counts.iter().max_by(|a, b| a.1.cmp(b.1)).unwrap();
    let key_2 = max.0 ^ b' ';

    let decoded = decode_cipher([key_0, key_1, key_2]);
    decoded.iter().map(|b| (*b as usize)).sum()
}
